...
class RegisterForm(forms.Form):
    username = forms.RegexField(label="Nazwa użytkownika", regex=r'^\w+$', min_length=2,
		max_length=30)
    ...
    def save(self):
        return get_user_model().objects.create_user(self.cleaned_data.get('username'),
			self.cleaned_data.get('email1'), self.cleaned_data.get('password1'),
			first_name=self.cleaned_data.get('first_name'), last_name=self.cleaned_data.get('last_name'))
